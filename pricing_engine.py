...
def calculate_price(commodity, quality_params, region, include_weather=True):
    """
    Calculate price for a commodity based on quality parameters, region and weather.

    Args:
        commodity (str): The commodity
        quality_params (dict): Quality parameters
        region (str): The region
        include_weather (bool, optional): Whether to include weather impact. Defaults to True.

    Returns:
        tuple: (final_price, base_price, quality_delta, location_delta, market_delta, weather_impact)
    """
    ...
    if include_weather:
        ...
        weather_analysis = analyze_weather_impact(commodity, region)

        if weather_analysis:
            # Apply storage impact
            storage_factor = weather_analysis["storage_impact"]
            weather_impact = (storage_factor - 1) * base_price

            # Adjust for seasonal risk
            if weather_analysis["seasonal_risk"]["risk_level"] == "high":
                weather_impact += base_price * 0.05  # 5% premium for high risk periods

    final_price = base_price + quality_delta + location_delta + market_delta + weather_impact
    ...
    return (final_price, base_price, quality_delta, location_delta, market_delta, weather_impact)
...
def calculate_price_curve(commodity, quality_param, region, min_value=None, max_value=None, points=20):
    ...
    for value in param_values:
        ...
        # Calculate price
        final_price, base_price, quality_delta, location_delta, market_delta, weather_impact = calculate_price(
...
if __name__ == "__main__":
    ...
    print(f"Weather Impact: ₹{weather_impact:.2f}")
